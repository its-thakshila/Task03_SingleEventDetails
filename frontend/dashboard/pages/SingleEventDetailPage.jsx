...
export default function SingleEventDetailPage() {
  ...
  // Guards to avoid race conditions / stale updates
  ...
  // Fetch event status — only depends on eventId, not on `event` to avoid extra churn
  useEffect(() => {
  ...
  // Fetch interested status EXACTLY once per eventId (do not depend on `event`)
  useEffect(() => {
    if (eventId == null) return;
    const controller = new AbortController();
  ...
  const handleToggleInterested = async () => {
    ...
    try {
      if (nextInterested) {
        ...
        // Only apply server values if this is the latest request
        ...
        <div className="event-left-section">
          ...
          <div className="event-details">
            ...
            <div className="detail-item">
              ...
              <div className="detail-content">
                <div className="detail-value">{Number(event.interested_count) || 0} People Interested</div>
              </div>
            </div>
          </div>
