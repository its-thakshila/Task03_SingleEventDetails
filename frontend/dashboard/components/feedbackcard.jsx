...
export default function FeedbackCard({ eventId }) {
  ...
  // load my rating + summary on mount
  useEffect(() => {
    if (!eventId) return;
    (async () => {
      try {
        const meRes = await fetch(`${API}/events/${eventId}/ratings/me`, {
          credentials: 'include'
        });
        if (meRes.status === 200) {
        ...
        const sumRes = await fetch(`${API}/events/${eventId}/ratings/summary`, {
          credentials: 'include'
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // refresh summary
      const s = await fetch(`${API}/events/${eventId}/ratings/summary`, {
        credentials: 'include'
      }).then(r => r.json());
      setSummary(s);
    } catch (e) {
            ...
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center space-x-3">
                <div className="flex items-center space-x-1">
                  ...
                  <span className="font-semibold text-gray-800">{summary.average ?? '—'}</span>
                </div>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-center space-x-3">
                <Send className={`w-5 h-5 ${isSubmitting ? 'animate-pulse' : 'group-hover:translate-x-1'} transition-transform duration-200`} />
